...
class FileServerApp:
    def __init__(self, root):
        ...
        @self.app.route('/view/<path:filename>', methods=['GET'])
        def view_file(filename):
            ...
            try:
                ...
                # 如果是目录，返回错误
                if os.path.isdir(absolute_requested):
                    return jsonify({'status': 'error', 'message': 'Cannot view directory'}), 400

                # 获取文件类型
                mime_type, _ = mimetypes.guess_type(absolute_requested)
                ...
                if mime_type.startswith('text/') or mime_type in [
                    ...
                    with open(absolute_requested, 'r', encoding='utf-8') as f:
                        content = f.read()
                    return Response(content, mimetype=mime_type)
                
                # 处理图片
                elif mime_type.startswith('image/'):
                    return send_file(absolute_requested, mimetype=mime_type)
                ...
                # 其他类型文件提供下载
                    ...
                    return send_file(absolute_requested, as_attachment=True)

            except Exception as e:
                self.log_message(f"View file error: {str(e)}")
                return jsonify({'status': 'error', 'message': str(e)}), 500
    ...
    def create_widgets(self):
        ...
        # 下载面板
        download_frame = tk.LabelFrame(self.root, text="文件访问", padx=10, pady=10)
        ...
        download_btn = tk.Button(button_frame, text="生成下载链接", command=self.generate_download_link)
        download_btn.pack(side=tk.LEFT, padx=5)
        
        view_btn = tk.Button(button_frame, text="生成访问链接", command=self.generate_view_link)
        ...
        self.view_link_var = tk.StringVar()
        tk.Label(download_frame, text="访问链接:").grid(row=3, column=0, sticky=tk.W)
        view_link_entry = tk.Entry(download_frame, textvariable=self.view_link_var, state='readonly')
        view_link_entry.grid(row=3, column=1, columnspan=2, sticky=tk.EW)
        ...
        self.popup_menu.add_command(label="查看", command=self.view_selected_item)
    ...
    def generate_download_link(self):
        ...
        # 获取相对于保存目录的路径
        current_save_dir = os.path.abspath(self.dir_entry.get())
        abs_file_path = os.path.abspath(file_path)
        relative_path = os.path.relpath(abs_file_path, start=current_save_dir)
        
        download_url = f"http://{host}:{port}/download?path={relative_path}"
        self.download_link_var.set(download_url)
        self.log_message(f"生成的下载链接: {download_url}")
    
    def generate_view_link(self):
        """生成查看文件内容的链接"""
        file_path = self.download_entry.get()
        if not file_path or not os.path.exists(file_path):
            messagebox.showerror("错误", "请选择有效的文件路径")
        ...
        if os.path.isdir(file_path):
            messagebox.showerror("错误", "不能查看目录内容")
            return
        ...
        view_url = f"http://{host}:{port}/view/{relative_path}"
        self.view_link_var.set(view_url)
        self.log_message(f"生成的查看链接: {view_url}")
    ...
    def view_selected_item(self):
        ...
        if item_values[0] == '文件夹':
            messagebox.showerror("错误", "不能查看文件夹内容")
            return
